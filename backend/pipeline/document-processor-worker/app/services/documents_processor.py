...
class DocumentsProcessor:
    ...
    def process_file(self, filepath: str, original_filename: str = None, session_id: str = None):
        start_time = time.time()
        filename = os.path.basename(filepath)
        print(f"Processing {filename} for session {session_id}...")
        chunks = self.chunker.chunk_document(filepath, filename)
        print(f" - Chunked into {len(chunks)} pieces.")
        self.graph_builder.process_chunks(chunks, original_filename=original_filename, session_id=session_id)
